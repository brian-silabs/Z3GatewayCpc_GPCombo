...
#ifdef SL_CATALOG_ZIGBEE_AARCH64_CORTEX_A72_GCC_12_3_0_MUSL_PRESENT
...
// sl_zigbee_ezsp_serial_write_byte() or sl_zigbee_ezsp_serial_read_byte()
...
// Define the termios bit fields modified by sl_zigbee_ezsp_serial_init
...
static sl_zigbee_ezsp_status_t ezspInternalSerialInit(bool ignoreErrors);
...
static sl_zigbee_ezsp_status_t ezspInternalSerialInit(bool ignoreErrors)
{
  ...
  if (SL_ZIGBEE_EZSP_SUCCESS == sl_zigbee_ezsp_setup_serial_port(&serialFd,
                                                                 errStr,
                                                                 ERR_LEN,
  ...
  if (strlen(errStr)) {
    sl_zigbee_ezsp_trace_event(errStr);
  }
  sl_zigbee_ezsp_serial_close();
  hostError = SL_ZIGBEE_EZSP_ERROR_SERIAL_INIT;
  return SL_ZIGBEE_EZSP_HOST_FATAL_ERROR;
}
...
sl_zigbee_ezsp_status_t sl_zigbee_ezsp_serial_read_byte(uint8_t *byte)
{
  ...
  status = sl_zigbee_ezsp_serial_read_available(&count);
  if (status == SL_ZIGBEE_EZSP_SUCCESS) {
    *byte = *inBufRd++;
